...
module elliptic_module
    ...
    contains
		...
		type( SurfaceType ) :: body
	    	...
	    	call eulerRot( 3, 1, 2, euler_a(1), euler_a(2), euler_a(3), body%COORDSYS )
		...
		print*, nx, na
	    	body%NN = (nx - 1) * na + 2
	    	body%NP = nx * na
	    	body%NVS = nx*na + na*(nx-1)
	    	body%bluff = .true.
	    	print*, body%NN! = (nx - 1) * na + 2
	    	print*, body%NP! = nx * na
	    	print*, body%NVS! = nx*na + na*(nx-1)
		...
		select case( axis )
		case( 2 )
			call eulerRot( 3, -90.d+0, body%NODESLOC )
		case( 3 )
			call eulerRot( 2, -90.d+0, body%NODESLOC )
		end select
		...
		call cPoints( body )
		call panelAreas( body )

		wvel_aux(:,1) = avel
		wvel_aux = matmul( body%COORDSYS, wvel_aux )

		do i = 1,body%NP
			body%CPUVW( :,i ) = bvel + cross( wvel_aux(:,1), body%CPXYZ( :,i ) - body%ORIGIN )
		end do
	...
	function ellipticNodes( length, max_radius, nx, na, nn ) result( nodes )
		...
		radius(1) = 0.d+0
        	do i=2,nx
        	        XOR(i) = - 0.5d+0 * length * cos((i)*dx)
        	        radius(i) = max_radius * sin((i)*dx)
        	end do
        	!do i=2,nx
        	!        XOR(i) = XOR(i-1) + dx
		!	ds = acos( abs(2.d+0 * XOR(i) / length) )
        	!        radius(i) = max_radius * sin(ds)
        	!end do
		XOR(nx+1) = 0.5d+0 * length
		radius(nx+1) = 0.d+0
	...
	subroutine ellipticPanels( body, nx, na )
		type( SurfaceType ), intent(inout) :: body
